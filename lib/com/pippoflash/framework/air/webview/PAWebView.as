package com.pippoflash.framework.air.webview 
{
	...
	public class PAWebView extends _PippoFlashBaseNoDisplay 
	{
		...
		public function loadHtmlFile(u:String, skipBaseFolder:Boolean=false, defaultWindowValues:Object=null):void {
			if (!_ready) {
		...
		public function callJavaScriptMethod(method:String):void {
			...
			Debug.debug(_debugPrefix, "Calling url " + u);
			_webView.loadURL(u);
		}
		 ...
		 */
		public function checkIfLocationIsInternalMessage(location:String):Boolean { // Returns true if event needs to be stopped from propagation
			Debug.debug(_debugPrefix, "Checking how to react  to location: " + location);
		 ...
		 */
		public function processJavaScriptMessage(location:String):Boolean {
			...
			if (location.indexOf(COMMAND_READY) == 0) {
				...
				Debug.debug(_debugPrefix, "JS CONSOLE> " + decodeURI(UText.removeTextUpTo(location, COMMAND_TRACE)));
				return true;
			}
		...
		private function setupHtmlIsReady():void {
			_loaded = true;
			callJavaScriptMethod("window.intializeAirFeatures(true, " + String(USystem.isDesktop()) + ", "+Debug.object(_defaultWindowValues)+")");
			UExec.next(PippoFlashEventsMan.broadcastInstanceEvent, this, EVT_READY, this);
		}
