...
// METHODS ///////////////////////////////////////////////////////////////////////////////////////
	...
	// CONTROL SOUNDS
		public static function playSound(id:String, loops:uint=1, listener:*=null, vol:Number=1, pan:Number=0):void { // This accepts the class id, or a custom ID used for sound lists
			...
			Debug.debug(_debugPrefix, "Play:",id,"\nloops",loops,"listener",listener,"volume",vol,"pan",pan);
			initializeSound(id, listener);
			startPlaying(id, 0, loops, listener, vol, pan);
		}
	...
	// GENERAL VOLUME
		public static function setGeneralVolume(v:Number):void {
			// if (_verbose) Debug.debug(_debugPrefix, "Set general volume (range is 0 to 1) to:",v);
			_soundMixerTransform.volume = v;
			SoundMixer.soundTransform = _soundMixerTransform;
		}
		public static function fadeGeneralVolumeTo(v:Number, frames:uint, callback:Function=null):void {
			if (getGeneralVolume()==v) return;
			doFadeVolumeTo(v, frames, callback);
		}
...
// UTY ///////////////////////////////////////////////////////////////////////////////////////	
		...
		private static function startPlaying(id:String, startAt:uint=0, loops:uint=1, listener:*=null, vol:Number=1, pan:Number=0):void {
			try {
		...
		// Fade GENERAL volume ////////////////////////////////////////////////////////////////////////////////
		private static var _targetVolume:Number;
		private static var _faderClip:MovieClip = new MovieClip(); // Needed for fade volume
		private static var _generalFadeCompleteCallback:Function;
		private static function doFadeVolumeTo(v:Number, framesDuration:uint=150, completeCallbackFunc:Function=null):void {
			Debug.debug(_debugPrefix, "Fading volume from " + getGeneralVolume() + " to " + v);
			resetGeneralVolumeFade();
			_targetVolume = v;
			_generalFadeCompleteCallback = completeCallbackFunc;
			var delta:Number = (v - getGeneralVolume()) / framesDuration;
			for (var i:uint=1; i<=framesDuration; i++) {
				_fadeVector.push(getGeneralVolume()+(delta*i));
			}
			_faderClip.addEventListener(Event.ENTER_FRAME, doStepVolumeFade);
		}
			private static function doStepVolumeFade(e:Event):void {
				if (_fadeVector.length) {
					setGeneralVolume(_fadeVector.splice(0,1)[0]);
				} else {
					Debug.debug(_debugPrefix, "Volume fade to " +_targetVolume+ " COMPLETE.");
					if (_generalFadeCompleteCallback) _generalFadeCompleteCallback();
					resetGeneralVolumeFade();
				}
			}
			private static function resetGeneralVolumeFade():void {
				_faderClip.removeEventListener(Event.ENTER_FRAME, doStepVolumeFade);
				_fadeVector = new Vector.<Number>();
				_generalFadeCompleteCallback = null;
			}
	}
...
// HELPER CLASSES ///////////////////////////////////////////////////////////////////////////////////////	
	...
	class USoundListener extends _PippoFlashBaseNoDisplayUMem {
		...
		// public var _durationMillisecs:uint;
		// public var _positionMillisecs:uint;
		...
		public  function onLoadComplete(e:Event):void {
			// trace("tsLOAD COMPLETEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE");
			if (USound._verbose) Debug.debug(_id, e);
			_loaded = true;
		...
		public function onSoundPlayStart():void {
			...
			UCode.callGroupMethod(_listeners, "onSoundPlayStart", _id);
		}
	}
